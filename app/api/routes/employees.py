...
async def create_employee(employee: EmployeeCreate, db: Session = Depends(get_db)):
    ...
    if existing_employee:
        # If the account is already verified/active, block duplicate registration
        ...
        # Account exists but is NOT verified -> regenerate token & re-send email
    ...
    # Construct verification link (frontend route could handle token)
    base_url = settings.frontend_url
    verification_link = f"{base_url}/verify-email?token={verification_token}&id={db_employee.id}"

    # Send email (logs instead if SMTP not configured)
...
@router.put("/{employee_id}/verify", response_model=EmployeeSchema)
